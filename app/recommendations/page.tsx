...
export default function RecommendationsPage() {
  ...
  const addToItineraryText = "Added to Itinerary!";
  ...
  useEffect(() => {
    const timer = setTimeout(() => {
      if (
        ...
        console.log(
          "⚠️ No recommendations found, redirecting to inspiration..."
        );
  ...
  const handleAddToItinerary = (rec: Recommendation) => {
    ...
    const uniqueId = `rec-${rec.id}-${Date.now()}-${Math.random()
      .toString(36)
      .substr(2, 9)}`;
    ...
    setTimeout(() => {
      if (availableRecommendations.length > 1) {
        ...
        const nextIndex =
          currentIndex >= availableRecommendations.length - 1
            ? 0
            : currentIndex;
  ...
  const handleCalendarImport = async (calendarUrl: string) => {
    ...
    try {
      ...
      setToastMessage(
        `✓ Successfully imported ${events.length} event(s) from calendar!`
      );
      setShowToast(true);
      setTimeout(() => setShowToast(false), 4000);
        ...
        <div className="h-screen bg-white dark:bg-gray-900 flex flex-col items-center justify-center px-4">
          <div className="text-center max-w-md">
            ...
            <p className="text-gray-600 dark:text-gray-400 mb-8">
              You&apos;ve added all suggested activities to your itinerary.
              Great job planning your trip!
            </p>
      ...
      <div className="absolute inset-0 z-0">
        <MapView
          ...
          onMarkerClick={(id) => {
            ...
            const index = availableRecommendations.findIndex(
              (r) => r.id === id
            );
            if (index !== -1) {
                ...
                <div className="flex-1 p-6 overflow-y-auto">
                  ...
                  {expandedCard.location && (
                    <div className="flex items-center gap-2 text-sm text-gray-600 dark:text-gray-400">
                      ...
                      <span>
                        Lat: {expandedCard.location.lat.toFixed(4)}, Lng:{" "}
                        {expandedCard.location.lng.toFixed(4)}
                      </span>
                    </div>
                  )}
                </div>
